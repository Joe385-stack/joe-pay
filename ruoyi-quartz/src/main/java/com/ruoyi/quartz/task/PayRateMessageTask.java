...
@Slf4j
@Component("payRateMessageTask")
public class PayRateMessageTask {
    ...
    public void sendMessage(){
        ...
        Integer messageType = MessageTypeEnum.PAY_RATE.getType();
        ...
        Date beginDate = DateTimeUtils.getBeginTimeOfDate(now);
        Date endDate = DateTimeUtils.getEndTimeOfDate(now);
    ...
    private void sendMessage(BotMessage message, PayRateVo payRateVo){
        ...
        String success = "0";
        String rate = "0.00%";
        String text = "【成功率通知】 商户【%s】在【%s】通道今日收款【%s】元,下单笔数【%s】笔,支付成功笔数【%s】笔,支付成功率【%s】";
        if(payRateVo != null){
